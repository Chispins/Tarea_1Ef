...
sigma = 0.0625
error = np.random.normal(0, sigma, size=(n, 1))
...
parte_1_sc = np.linalg.inv(X_sin_constante.T @ X_sin_constante) @X_sin_constante.T
parte_1 =np.linalg.inv(X.T @ X) @ X.T
beta_gorro_MC = []
beta_gorro_sc_MC = []
for i in range(1000):
    itero = y_real + np.random.normal(0, sigma, size=(n, 1))
    itero_beta_gorro = parte_1 @ itero
    itero_beta_gorro_sc = parte_1_sc @ itero
...
Betas_MC_sc_std = [beta_gorro_sc_MC_np[:, 0].std(),beta_gorro_sc_MC_np[:, 1].std()]
Betas_MC = [beta_gorro_MC_np[:, 0].mean(), beta_gorro_MC_np[:, 1].mean(), beta_gorro_MC_np[:, 2].mean()]
Betas_MC_std = [beta_gorro_MC_np[:, 0].std(), beta_gorro_MC_np[:, 1].std(), beta_gorro_MC_np[:, 2].std()]
